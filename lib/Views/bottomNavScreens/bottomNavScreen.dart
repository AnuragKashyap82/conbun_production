...
void initState() {
    ...
    if (widget.currentTab == 0) {
      ...
      currentScreen = MyAppointmentScreen();
...
Widget build(BuildContext context) {
    return StreamBuilder<List<ConnectivityResult>>(
      stream: _connectivity.onConnectivityChanged.map((results) => results),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return  _buildHasConnectionUi();
        }
        // Determine if there's no connectivity
        final isConnected =
        ...
        // Return your BottomNavScreen UI
        return _buildHasConnectionUi();
      },
    );
  }
